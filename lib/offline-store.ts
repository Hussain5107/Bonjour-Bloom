import type {AppData} from './progress';
...
export async function loadAppState(){if(typeof indexedDB==='undefined')return null;const db=await openDb(),tx=db.transaction(STATE,'readonly');return await requestValue(tx.objectStore(STATE).get('app')) as AppData|undefined||null}
...
export async function migrateLegacyState(){const current=await loadAppState();if(current)return current;const raw=localStorage.getItem('bonjour-bloom');if(!raw)return null;try{const state=JSON.parse(raw) as AppData;await saveAppState(state);return state}catch{return null}}
